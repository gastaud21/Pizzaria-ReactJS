...
function App() {
  ...
  const listaPizzas = pizzas.map((pizza, i) => (
    <tr key={pizza.nome}>
      ...
      <td><i className='bi bi-search fs-4 text-info' style={{ cursor: 'pointer' }} title='Ver Detalhes' onClick={() => mostraPizza(i)}></i>
      <i className='bi bi-trash3 fs-4 text-danger' style={{cursor: 'pointer'}} title='Excluir Pizza' onClick={() => excluiPizza(i)}></i></td>
    </tr>
  ))
    ...
    <div className="container-fluid">
      ...
      <Modal open={open} onClose={() => setOpen(false)} center>
        <div className="card">
          ...
          {watch("foto") &&
            <img src={watch("foto")} alt='foto da pizza' className="rounded mx-auto d-block" width={240} height={200} />
          }
        </div>
      </Modal>
      <Modal open={open2} onClose={() => setOpen2(false)} center>
        <div class="card">
          <img src={modalFoto} className="card-img-top" width={400} height={320}/>
            <div class="card-body">
